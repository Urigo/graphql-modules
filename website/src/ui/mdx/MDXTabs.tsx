...
import { MDXTabsCurrentTabContext } from './MDXTabsCurrentTabContext';
import React, {
  Children,
  createContext,
  ReactNode,
...
const MDXTabsCurrentTabsContextProvider: React.FunctionComponent = ({
  children,
}) => {
  ...
  return (
    <MDXTabCurrentTabsContext.Provider value={{ value, addTab }}>
      {children}
    </MDXTabCurrentTabsContext.Provider>
  );
};

const MDXTabsRenderer = ({
  children,
  namespace,
}: {
  namespace: string;
