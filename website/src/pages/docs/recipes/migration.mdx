...
## ExecutionContext is now disabled by default

In v2.X, the `ExecutionContext` was enabled by default, in v3.X, it is not.
If you want to use it, you need to enable it explicitly.

The Execution Context uses `async_hooks` under the hood, which is only available on Node. 
To make `graphql-modules` platform agnostic, we decided to make it opt-in.
